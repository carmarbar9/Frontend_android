...
class _ProvidersPageState extends State<ProvidersPage> {
  ...
  void _navigateToEditProvider(Proveedor proveedor) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => ProviderFormPage(proveedor: proveedor)),
    );
    if (result == true) {
      setState(() {
        _loadProveedores();
                        ...
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          if (_searchQuery != null && _searchQuery!.isNotEmpty) {
                            ...
                            if (RegExp(r'^\d+$').hasMatch(query)) {
                              proveedores = proveedores.where((p) => p.telefono != null && p.telefono!.startsWith(query)).toList();
                            } else {
                              proveedores = proveedores.where((p) => p.name != null && p.name!.toLowerCase().startsWith(query.toLowerCase())).toList();
                            }
                          }
  ...
  Widget _buildProviderCard(Proveedor proveedor) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(proveedor.name ?? '', style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold, fontFamily: 'TitanOne', color: Colors.white)),
          ...
          Row(
            children: [
              Expanded(child: _buildFlatWhiteButton(icon: Icons.edit, label: "Editar", onPressed: () => _navigateToEditProvider(proveedor))),
              const SizedBox(width: 10),
              Expanded(child: _buildFlatWhiteButton(icon: Icons.delete, label: "Eliminar", onPressed: () => _deleteProvider(proveedor))),
            ],
          ),
